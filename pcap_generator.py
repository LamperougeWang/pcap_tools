...
for count in range(flow_count):
    for idx in range(3):
        ...
        if new_pkt[IP].src == "101.6.30.128":
            ...
            new_pkt[TCP].dport = port_dst_array[count]
        else:
            new_pkt[IP].src = ip_dst_array[count]
            new_pkt[IP].dst = ip_src_array[count]
            new_pkt[TCP].sport = port_dst_array[count]
            new_pkt[TCP].dport = port_src_array[count]
...
for count in range(packet_count):
    ...
    for idx in range(8):
        ...
        if new_pkt[IP].src == "101.6.30.128":
            ...
            new_pkt[TCP].dport = port_dst_array[num_rnd]
        else:
            new_pkt[IP].src = ip_dst_array[num_rnd]
            new_pkt[IP].dst = ip_src_array[num_rnd]
            new_pkt[TCP].sport = port_dst_array[num_rnd]
            new_pkt[TCP].dport = port_src_array[num_rnd]
